...
with st.sidebar:
    ...
    if opts_per_line == 4:
        default_char_limit = 20
    elif opts_per_line == 3:
        default_char_limit = 25
    else:
        default_char_limit = 35
    opt_char_limit = st.slider("Max chars per option (threshold)", 10, 80, default_char_limit,
        help="Each option's text must be shorter than this to share a line. Shorter = stricter (more solo lines). This is per-option, not total.")
...
def parse_questions(doc):
    ...
    def is_question_start(text):
        ...
        return bool(
            ...
            re.match(r'^Q\.\s*\d+[\.\s]', text, re.IGNORECASE)
        )
...
def process_question_block(block):
    ...
    for pattern in [r'प्रश्न\s+(\d+)', r'^Q\.\s*(\d+)[\.\s]', r'^(\d+)\.', r'^(\d+)\s+']:
...
def layout_options(opts, max_per_line=2, char_limit=68):
    """
    Group options onto lines smartly:
    - char_limit is treated as the max characters allowed per individual option
      (including its key label like '(a) ')
    - Options share a line only when EVERY option in the group fits within
      char_limit characters on its own — this ensures no option ever wraps
      mid-column, causing the next option to start at the wrong position.
    - Falls back gracefully: tries max_per_line first, then fewer, then 1.
    """
    result = []
    i = 0
    n = len(opts)
    while i < n:
        ...
        for k in range(max_per_line, 1, -1):
            if i + k <= n:
                # Each option must individually fit within char_limit
                all_fit = all(
                    len(f"{opts[i+j]['key']} {opts[i+j]['text']}") <= char_limit
                    for j in range(k)
                )
                if all_fit:
                    best = k
...
def render_q_preview(q):
    ...
    for idx, group in enumerate(option_groups):
        ...
        if n_cols == 1:
            # Single option — just a plain div, no grid needed
            cell_html = (
                f"<div style='font-size:{opt_font}pt;'>"
                f"{group[0]['key']} {group[0]['text']}</div>"
            )
            if show_correct_inline and is_last:
                opts_html += (
                    f"<div style='display:flex;justify-content:space-between;"
                    f"margin-left:{l2px}px;font-size:{opt_font}pt;'>"
                    f"<span>{group[0]['key']} {group[0]['text']}</span>"
                    f"<span style='font-weight:900;font-size:{opt_font+1.5}pt;'>{q['correct']}</span>"
                    f"</div>"
                )
            else:
                opts_html += (
                    f"<div style='margin-left:{l2px}px;font-size:{opt_font}pt;'>"
                    f"{group[0]['key']} {group[0]['text']}</div>"
                )
        else:
            # Multiple options — use CSS grid so each option occupies exactly
            # 1/n_cols of the width. Even if text wraps inside a cell, the
            # next option always starts at a clean column boundary.
            cells = "".join(
                f"<div style='min-width:0;word-break:break-word;'>"
                f"{o['key']} {o['text']}</div>"
                for o in group
            )
            grid_div = (
                f"<div style='display:grid;grid-template-columns:repeat({n_cols},1fr);"
                f"margin-left:{l2px}px;font-size:{opt_font}pt;gap:0 4px;'>"
                f"{cells}"
            )
            if show_correct_inline and is_last:
                # Append the answer as an extra right-aligned cell
                grid_div += (
                    f"<div style='grid-column:{n_cols};text-align:right;font-weight:900;"
                    f"font-size:{opt_font+1.5}pt;'>{q['correct']}</div>"
                )
                # But we used n_cols columns; answer goes after — use flex wrapper instead
                # Reset and do flex approach for last line with answer
                grid_div = (
                    f"<div style='display:flex;justify-content:space-between;"
                    f"align-items:flex-start;margin-left:{l2px}px;font-size:{opt_font}pt;'>"
                    f"<div style='display:grid;grid-template-columns:repeat({n_cols},1fr);"
                    f"flex:1;gap:0 4px;'>{cells}</div>"
                    f"<span style='font-weight:900;font-size:{opt_font+1.5}pt;"
                    f"white-space:nowrap;margin-left:6px;'>{q['correct']}</span>"
                    f"</div>"
                )
            else:
                grid_div += "</div>"
