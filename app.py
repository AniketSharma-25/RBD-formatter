...
def verify_otp(email, code):
    ...
    if row and row[0] == code:
        stored_expiry = datetime.datetime.fromisoformat(row[1])
...
def log_activity(user_email, action, details=""):
    ...
    c = conn.cursor()
    now = datetime.datetime.now().isoformat()
    c.execute("INSERT INTO logs (user_email, action, details, timestamp) VALUES (?, ?, ?, ?)",
              (user_email, action, details, now))
    conn.commit()
    conn.close()
...
def get_all_users():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT email, is_admin, can_format FROM users ORDER BY email")
    rows = c.fetchall()
    conn.close()
    return [{"email": r[0], "is_admin": bool(r[1]), "can_format": bool(r[2])} for r in rows]

def update_user_permission(email, can_format):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("UPDATE users SET can_format = ? WHERE email = ?", (can_format, email))
    conn.commit()
    conn.close()
...
def send_otp_email(email, code):
    ...
    subject = "Your OTP for RBD Formatter"
    ...
    with st.form("login_form"):
        ...
        if submitted and email:
            ...
            if send_otp_email(email, otp):
                ...
                st.error("Could not send OTP. Please try again later.")

    if "auth_email" in st.session_state:
        with st.form("otp_form"):
            code = st.text_input("Enter OTP", type="password")
            submitted_otp = st.form_submit_button("Verify")
            if submitted_otp and code:
                if verify_otp(st.session_state["auth_email"], code):
...
def add_run(para, text, bold=False, size_pt=8, italic=False):
    ...
    r.font.size = Pt(size_pt)
    r.font.name = FONT_DOCX
    if 'char_spacing' in globals() and char_spacing > 0:
        set_char_spacing(r, char_spacing)
    return r
...
# =============================================================================
# MAIN APP WITH AUTHENTICATION & PERMISSIONS
# =============================================================================
def main_app():
    ...
    can_format = st.session_state["can_format"]

    st.set_page_config(page_title="RBD Formatter", layout="wide")
    st.title("📚 RBD Publication – Smart Formatter")

    # Global variables used by formatting functions – we'll set them based on role
    global page_width, page_height, top_margin, bottom_margin, left_margin, right_margin
    global num_columns, auto_fill
    global q_font, level1_indent, level2_indent
    global opt_font, opt_bold, ans_font, ans_bold, expl_font
    global line_spacing, para_spacing, char_spacing
    global opts_per_line, opt_char_limit
    global header_template, book_name, header_font, header_bold, header_bg, header_align
    global page_num_pos, hide_on_first
    global show_correct_inline, show_separator, expl_bullet, expl_bg

    # ---------- SIDEBAR ----------
    with st.sidebar:
        st.header(f"👤 {user_email}")
        if is_admin:
            st.success("Admin mode – full control")
            # Admin sees all formatting controls (copied from original)
            st.header("📄 Page Design")
            page_width = st.number_input("Page Width (inches)", 5.0, 12.0, 7.0, 0.1)
            page_height = st.number_input("Page Height (inches)", 6.0, 14.0, 9.0, 0.1)
            top_margin = st.number_input("Top Margin (inches)", 0.2, 1.0, 0.4, 0.05)
            ...
            st.info("Regular user – using default settings")
            # Default values for non‑admin users (they cannot change them)
            page_width = 7.0
            page_height = 9.0
            top_margin = 0.4
            bottom_margin = 0.4
            ...
            opts_per_line = 2
            opt_char_limit = 68
            header_template = "{book_name} | {chapter_title} | पृष्ठ {page}"
            book_name = "RBD PUBLICATION"
            header_font = 11.0
            header_bold = True
            header_bg = True
            header_align = "Center"
            page_num_pos = "Bottom Right"
            hide_on_first = False
            show_correct_inline = True
    ...
    if uploaded_file:
        ...
        with st.spinner("Parsing questions..."):
            ...
            chapter_title = extract_chapter_title(doc)
        st.success(f"✅ {len(questions)} questions parsed!")
        log_activity(user_email, "upload", f"Uploaded file with {len(questions)} questions")
        ...
        st.markdown("---")
        # Download buttons are only shown if user has format permission
        if can_format or is_admin:
                ...
                if st.button("🚀 Generate DOCX"):
                    with st.spinner("Generating DOCX..."):
                        ...
                        with open(filename, "rb") as f:
                            st.download_button("📥 Download DOCX", f, filename,
                                               "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
                        log_activity(user_email, "download_docx", filename)
                        st.success("🎉 DOCX ready!")
            with col2:
                if st.button("📑 Preview PDF"):
                    with st.spinner("Generating PDF preview..."):
                        ...
                        st.download_button("📥 Download PDF", pdf_buffer,
                                           file_name="Formatted_Output.pdf", mime="application/pdf")
                        log_activity(user_email, "download_pdf", "Formatted_Output.pdf")
                        st.success("🎉 PDF preview ready!")
        else:
            st.info("🔒 You have preview‑only access. To download formatted files, ask the admin to grant you permission.")
    ...
    if is_admin:
        with st.expander("👥 User Management"):
            ...
            for user in users:
                if user["email"] == user_email:
                    continue   # skip admin's own row
                col1, col2 = st.columns([3, 1])
                with col1:
                    st.write(user["email"])
                with col2:
                    new_val = st.checkbox("Can format", value=user["can_format"],
                                          key=f"perm_{user['email']}")
                    if new_val != user["can_format"]:
                        update_user_permission(user["email"], new_val)
        ...
        with st.expander("📊 Activity Logs"):
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute("SELECT timestamp, user_email, action, details FROM logs ORDER BY timestamp DESC LIMIT 100")
            logs = c.fetchall()
            conn.close()
            if logs:
                st.table(logs)
